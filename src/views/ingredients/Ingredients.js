...
import AddIngredientButton from './ingredientsComponents/AddIngredientButton';
import Categories from './ingredientsComponents/Categories';
import IngredientList from './ingredientsComponents/IngredientList';
import SearchIngredientsBar from './ingredientsComponents/SearchIngredientsBar';
function Ingredients() {
    ...
    const [ingredientListForSearchBarFiltering,setingredientListForSearchBarFiltering] = useState(INGREDIENTS)
    let filteredIngredientList = [...INGREDIENTS]
    const searchBarFiltering = (e) => {
        const keyword = e.target.value;
    
        if (keyword !== '') {
          const results = ingredientListForSearchBarFiltering.filter((ingredient) => {
            return ingredient.nomIng.toLowerCase().startsWith(keyword.toLowerCase());
